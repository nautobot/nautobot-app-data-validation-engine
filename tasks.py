...
def _ensure_creds_env_file(context):
    ...
    if not os.path.exists(os.path.join(context.nautobot_data_validation_engine.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_data_validation_engine.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_data_validation_engine.compose_dir, "creds.env"),
        )
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_data_validation_engine'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_data_validation_engine")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_data_validation_engine.migrations"
