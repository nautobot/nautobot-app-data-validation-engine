...
def _is_compose_included(context, name):
    return (
        f"docker-compose.{name}.yml"
        in context.nautobot_data_validation_engine.compose_files
    )
...
def docker_compose(context, command, **kwargs):
    ...
    for compose_file in context.nautobot_data_validation_engine.compose_files:
        compose_file_path = os.path.join(
            context.nautobot_data_validation_engine.compose_dir, compose_file
        )
...
def build(context, force_rm=False, cache=True):
    ...
    print(
        f"Building Nautobot with Python {context.nautobot_data_validation_engine.python_ver}..."
    )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    dockerfile_path = os.path.join(
        context.nautobot_data_validation_engine.compose_dir, "Dockerfile"
    )
    base_image = (
